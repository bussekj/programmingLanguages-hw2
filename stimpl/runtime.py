...
class State(object):
    ...
    def get_value(self, variable_name: str) -> Any:
        return self.value if self.variable_name == variable_name else self.next_state.get_value(variable_name)
...
def evaluate(expression: Expr, state: State) -> Tuple[Optional[Any], Type, State]:
    match expression:
        ...
        case Multiply(left=left, right=right):
            ...
            if left_type != right_type:
                raise InterpTypeError(f"""Mismatched types for Multiply: Cannot multiply {right_type} and {left_type}""")
            match left_type:
                case Integer() | FloatingPoint():
                    result = left_result * right_result
                case _:
        ...
        case Divide(left=left, right=right):
            ...
            if left_type != right_type:
                raise InterpTypeError(f"""Mismatched types for Divide: Cannot divide {right_type} by {left_type}""")
            match left_type:
                case Integer() | FloatingPoint():
                    if right_result == 0:
                        raise InterpMathError("Cannot divide by 0")
                    result = left_result / right_result
                case _:
